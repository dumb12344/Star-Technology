...
let P = {
  ...
  formMultiblock: function (scene, controllerPosition) {
    scene
      .world()
      .modifyBlockEntity(controllerPosition, $MetaMachineBlockEntity, (be) => {
  ...
  hideSectionAndRemoveBlocks: function (scene, blocks, direction) {
    let section = sectionFromPositions(scene, blocks);
   ...
   * @param {(Internal.SceneBuilder, Internal.SceneBuildingUtil) => void} scene
   */
  defineScene: function (sceneName, scene) {
    global.ponderScenes[sceneName] = scene;
  },
