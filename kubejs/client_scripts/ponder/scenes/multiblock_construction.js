...
let { electric_blast_furnace } = global.ponderMultis;

/**
 * @param {Internal.SceneBuilder} scene
 * @param {Internal.SceneBuildingUtil} util
 */
function scene(scene, util) {
  ...
  revealBlocks(
    scene,
    [{ pos: [2, 4, 2], state: block("gtceu:lv_muffler_hatch", Direction.UP) }],
    Direction.DOWN
  );
  ...
  revealBlocks(
    ...
    [
      ...
      { pos: [1, 1, 1], state: block("gtceu:lv_output_bus", Direction.NORTH) },
    ],
    Direction.SOUTH
  );
  scene.idle(20);
  ...
  revealBlocks(
    scene,
    [
      { pos: [1, 1, 2], state: block("gtceu:lv_input_hatch", Direction.WEST) },
      { pos: [1, 1, 3], state: block("gtceu:lv_output_hatch", Direction.WEST) },
    ],
    Direction.EAST
  );
  scene.idle(20);
  ...
  revealBlocks(
    ...
    steps[2]
      .concat(steps[3])
      .map((b) => ({ pos: b.pos, state: block("gtceu:nichrome_coil_block") })),
    Direction.NORTH
  );
  scene.idle(20);
  formMultiblock(scene, ebf.controller);
...
defineScene("multiblock_construction", scene);
