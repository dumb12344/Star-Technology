...
let { classic_stargate } = global.ponderMultis;
...
function scene(scene, util) {
  ...
  revealBlocks(scene, [
    {
      ...
      state: block("minecraft:oak_fence"),
    },
  ]);
  ...
  scene
    ...
    .showText(120)
    .text(
      "Right-click the DHD and start dialing in the address of your destination. 32,000,001 FE are needed to open a Stargate to §5Abydos§r."
    );
  ...
  scene.addInstruction(
    new PonderTickingInstruction(true, 120)
      .onFirstTick((scene) => {
        tickCount = 0;
...
defineScene("classic_stargate", scene);
